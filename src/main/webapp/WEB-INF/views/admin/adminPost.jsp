...
<body>
							...
							<td>
								<div class="nickNameBox">
									...
									<div class="nickName nickNameLayer${post.post_code }">
										<ul>
											<li class="search"><a href="adminPost?search_type=search_nick&
	...
	<div style="display: block; text-align: center;">
		<c:if test="${PAGE.startPage != 1 }">
			<a href="adminPost?cntPage=${PAGE.startPage - 1 }&search_type=${TYPE}&search_text=${TEXT}">&lt;</a>
		</c:if>
		<c:forEach begin="${PAGE.startPage }" end="${PAGE.endPage }" var="p">
			<c:choose>
				...
				<c:when test="${p != PAGE.cntPage }">
					<a href="adminPost?cntPage=${p }&search_type=${TYPE}&search_text=${TEXT}">${p }</a>
				</c:when>
			</c:choose>
		</c:forEach>
		<c:if test="${PAGE.endPage != PAGE.lastPage}">
			<a href="adminPost?cntPage=${PAGE.endPage+1 }&search_type=${TYPE}&search_text=${TEXT}">&gt;</a>
		</c:if>
	</div>
