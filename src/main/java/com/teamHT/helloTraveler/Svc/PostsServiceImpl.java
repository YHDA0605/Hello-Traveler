...
public class PostsServiceImpl implements IPostsService {
	...
	public void selectIndex(Model model) {
		List<PostsDTO> goodList = posConDao.selectIndex();
		
		model.addAttribute("HLIST",goodList);
	}
	...
	public String getPosts(PagingVO paging, String common_code, String cntPage, String search_type, String search_text, Model model) {
	      ...
	      total = posConDao.countPosts(search_common_code, search_title, search_nick);
	      paging = new PagingVO(total, Integer.parseInt(cntPage));
	      
	      model.addAttribute("LIST", posConDao.getGoodCounts(paging.getStart(), paging.getEnd(), search_common_code, search_title, search_nick));
	...
	@Override
	public String getadminPosts(PagingVO paging, String cntPage, String search_type, String search_text, Model model) {
	      int total = 0;
	      String search_common_code = "";
	      String search_nick = "";
	      String search_title = "";
	      ...
	      model.addAttribute("LIST", posConDao.getadminPosts(paging.getStart(), paging.getEnd(), search_title, search_nick));
	      model.addAttribute("PAGE", paging);
	      model.addAttribute("TYPE", search_type);
	      model.addAttribute("TEXT", search_text);
	...
	public String insertPost(PostsDTO post, String dbImg) {
		...
		return "redirect:/postContent?common_code=" + post.getCommon_code() + "&POST_CODE=" + post.getPost_code();
	...
	public String deletePost(String common_code, String post_code) {
		...
		if(post.getRe_step() > 1) {
			return  "redirect:/postContent?common_code=" + post.getCommon_code() + "&POST_CODE=" + post.getRe_post_code();
		}
		
		return "redirect:/post?common_code=" + post.getCommon_code();
	}
	...
	public String insertComent(PostsDTO post) {
		...
		return "redirect:/postContent?common_code=" + post.getCommon_code() + "&POST_CODE=" + firstPostCode;
	}
